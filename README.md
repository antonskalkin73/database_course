...
Проектирование схемы базы данных для сети ресторанов. Каждый ресторан работает отдельно и не зависим друг от друга. В качестве контрольного примера будет только 1 ресторан (таблица с ресторанами будет иметь только 1 запись)

Ресторан представляет собой обычное заведение, не относящееся к категории фастфуда. Подразумевается наличие официантов. Все точки сети имеют общее меню и клиентскую базу. Клиенты могут получить скидку при соблюдении определённых условий.

Меню разделено на категории. Категории могут иметь ограничения по возврату.
...
## Логичный план реализации БД
1. Создание схемы данных с ограничения и ключами
2. Создание триггеров
3. Заполнение данными в сущностях без многие ко многим
4. Генерация данных в таблицах с многие ко многим
